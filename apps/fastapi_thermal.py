...
~~~~~~~~~~~~~~~~~~~~~~~
Dedicated FastAPI web shell for the SAFIR **Thermal** Results Viewer.

Architecture
------------
- FastAPI (port 8000) – landing page, case picker, Bokeh embedding
- Bokeh Server (port 5006) – interactive 2-D thermal plots (``/thermal_viewer``)
...
BOKEH_THERMAL_URL      Full URL of the Bokeh thermal viewer app.
...
BOKEH_THERMAL_URL: str = os.environ.get(
    "BOKEH_THERMAL_URL", "http://localhost:5006/thermal_viewer"
)
...
app = FastAPI(title="SAFIR Thermal Results Viewer")
...
async def landing_page(
    ...
    """Render the thermal viewer landing page.
        ...
        if case not in case_map:
            ...
            if not Path(db_path).is_file():
                ...
                try:
                    ...
                    error_message = (
                        f"Could not connect to the Bokeh thermal server at "
                        f"{BOKEH_THERMAL_URL}. "
                        "Make sure it is running (see launch_thermal.bat)."
                    )
                    ...
                    error_message = (
                        f"Failed to load the thermal viewer for case '{case}'. "
