"""
bokeh_thermal/main.py
~~~~~~~~~~~~~~~~~~~~~
SAFIR 2-D Thermal Results Viewer – Bokeh Server application.

Launch alongside the structural viewer with::
...
The thermal app is then reachable at ``http://localhost:5006/main``.

To specify the database path set the environment variable or pass it via
the Bokeh ``--args`` flag::

    set SAFIR_DB_PATH=C:\\path\\to\\Raw.db
    bokeh serve bokeh_thermal/main.py --show --allow-websocket-origin=*
...
All SQL queries are kept in ``db_queries_thermal.py``.
"""
...
# ---------------------------------------------------------------------------
# Add the project root to sys.path so db_queries_thermal can be imported
...
import db_queries_thermal as dbq  # noqa: E402
